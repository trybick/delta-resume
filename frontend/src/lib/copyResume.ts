...
const nameHtml = (text: string): string =>
  `<p style="text-align: center; font-size: 16pt; font-weight: bold; margin: 0 0 4pt 0;">${escapeHtml(text)}</p>`

const headerLineHtml = (text: string): string =>
  `<p style="text-align: center; margin: 0 0 2pt 0;">${escapeHtml(text)}</p>`

const headingHtml = (text: string): string =>
  `<p style="font-size: 12pt; font-weight: bold; border-bottom: 1px solid #999; margin: 10pt 0 4pt 0;">${escapeHtml(text)}</p>`

const subheadingHtml = (text: string): string =>
  `<p style="font-weight: bold; margin: 6pt 0 2pt 0;">${escapeHtml(text)}</p>`

const paragraphHtml = (text: string): string =>
  `<p style="margin: 0 0 4pt 0;">${escapeHtml(text)}</p>`

const listItemHtml = (text: string): string =>
  `<li style="margin: 0 0 2pt 0;">${escapeHtml(text)}</li>`
...
export const buildStructuredResumeHtml = (
  lines: string[],
...
export const copyResumeRichText = async (
  ...
  const html = structure
    ? buildStructuredResumeHtml(lines, structure)
